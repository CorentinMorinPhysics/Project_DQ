...
power_supply_brand = 'Yoko'

if power_supply_brand=='Yoko':
    set_volt_lin = 'SOUR:LEV '
elif power_supply_brand=='Agilent':
    set_volt_lin = 'VOLTage '

#%%
def go_to_volt_power(beg,target,device):
    if beg==target:
        device.write(set_volt_lin+str(beg))
        device.write('OUTPut ON') 
    else:
        temp_V = np.linspace(beg,target,10)
        device.write(set_volt_lin+str(temp_V[0]))
        device.write('OUTPut ON')   
        for k in range(len(temp_V)):
            device.write(set_volt_lin+str(temp_V[k]))
            time.sleep(0.5)
...
powersupply_address = 'GPIB1::11::INSTR' #Put here the Visa address of the high voltage generator
...
#     powersupply.write(set_volt_lin+str(list_high_V[i]))
